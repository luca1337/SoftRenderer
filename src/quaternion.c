...
doge_quat_t doge_quat_mul(doge_quat_t q1, doge_quat_t q2)
{
    ...
    q.v = sum_vec3(sum_vec3(scale_vec3_float(q1.v, q2.w), scale_vec3_float(q2.v, q1.w)), vec3_cross(q1.v, q2.v));
    return q;
}
...
vec3_t doge_quat_rotated_vec(vec3_t v, doge_quat_t q)
{
    ...
    return sum_vec3(sum_vec3(scale_vec3_float(cross, q.w * 2), v), scale_vec3_float(vec3_cross(q.v, cross), 2));
}